...
This landscape only measures for-profit companies. 

They may or may not have raised venture capital. This landscape does not measure the number of open source technologies or distinct commercial products within a category, so the cumulative activity that defines a segment is often much broader than the landscape suggests. 
Developers, by their very nature, are creative individuals, so when they are unable to find a vendor offering that matches well with something they are seeking, it's not uncommon for them to start a new project that satisfies their needs.

The landscape does not include the (massive) system integrator and consulting industry. However, it does include hosted offerings that can directly impact developer productivity and technology choices, such as Stack Overflow.
